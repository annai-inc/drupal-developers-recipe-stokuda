...
翻訳文字列は管理UIから動的に翻訳を書き換えることもできますが、もちろんコードで管理することもできます。
...
2. 本セクションで `About` という文字列を翻訳しました。この文字列は `/admin/help/node` など他のモジュールのヘルプページでも使われており、コアのデフォルトの翻訳に `アバウト` という既存の定義が存在します。つまり、本セクションの実装により他のUIに対してて副作用が発生しています。この副作用が解消されるように翻訳の実装を修正してください。
